...
import { AlertCircle, Check, Mountain, Eye, EyeOff } from "lucide-react";
...
function SignupForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setDone(true);
      setTimeout(() => router.push(callbackUrl ? `/login?callbackUrl=${encodeURIComponent(callbackUrl)}` : "/login"), 2500);
    } catch (err) {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-transparent p-4 text-cream font-sans">
        ...
        >
          ...
          <p className="text-cream/60 mb-8 font-light">Account created successfully. Please log in to continue.</p>
          <button onClick={() => router.push(callbackUrl ? `/login?callbackUrl=${encodeURIComponent(callbackUrl)}` : "/login")} className="px-8 py-4 bg-burnt-orange hover:bg-[#d98542] text-white rounded-xl font-semibold transition-all shadow-[0_0_20px_rgba(198,122,60,0.3)] hover:shadow-[0_0_30px_rgba(198,122,60,0.5)]">
            Go to Login
          </button>
        </motion.div>
      </main>
          ...
          <div className="w-full max-w-[340px] mx-auto relative z-10 py-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative group">
                <input
                  ...
                  className="peer w-full bg-transparent border-b border-cream/10 py-2 pr-8 text-white focus:outline-none focus:border-burnt-orange transition-colors placeholder-transparent font-light text-sm"
                ...
                <button
                  ...
                  className="absolute right-0 top-2 text-cream/40 hover:text-white transition-colors"
                >
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-col gap-2.5">
                <button onClick={handleGoogleSignUp} type="button" className="w-full flex items-center justify-center gap-2 py-3 border border-cream/10 rounded-lg hover:border-cream/30 hover:bg-cream/5 transition-all text-sm font-medium text-cream/80 shadow-sm">
